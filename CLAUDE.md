...
**Files to fill in:**
- [ ] Replace `test-JS-noBuild` everywhere in this file with this project's name (the repo name is usually fine: e.g. `my-app`, `react-thing`, `cool-project`).
- [ ] Replace `(fill in a one-line description)` with 1-2 sentences about what this project is and what it does.
- [ ] Replace `ESM (type: module)` with the technology stack (e.g. `vanilla JS + Webpack`, `React + Vite`, `Next.js`, `SvelteKit`).
...
- [ ] Replace `npm run test:run` with the command CI runs to test (e.g. `npm test`, `npm run test:ci`, `vitest run`).
...
- [ ] `.github/workflows/test.yml` exists and runs `npm run test:run`.
