...
This file documents the project-specific commands, conventions, and quirks for **test-JS-noBuild**. It supplements `routine-base.md` (which has the universal discipline) with project-specific details Claude runs need to know.
...
**Variant:** gen-src-manifest.cjs (either `gen-src-manifest.js` for CommonJS projects or `gen-src-manifest.cjs` for ESM projects with `"type": "module"` in package.json)
...
>
> - "test-JS-noBuild is a single-page app; routing is hash-based via {{ROUTER_FILE}}."
...
>
> - "Run `npm run test:run` before opening the PR — CI will catch failures but the agent should not rely on CI to find them."
