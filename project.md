...
- **No build step.** Vanilla ES modules loaded directly by the browser — no
  bundler, no framework, no runtime npm dependencies. The repo's files are the
  files the browser gets, so anything requiring compilation is out.
